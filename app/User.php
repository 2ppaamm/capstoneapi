                                    ...
                                    CanResetPasswordContract
{
   ...
   public function scopeProfile($query, $id) { 
    ...
    // 2019-06-04 if diagnostic, check skills passed and calculate maxile
        $test_tracks = [];
        foreach ($test->questions as $question){
            array_push($test_tracks,$question->skill->tracks()->first()->id);
        }
        if ($test->diagnostic){
            $user_maxile = 100+$this->testedTracks()->whereIn('id',$test_tracks)->avg('track_maxile');
        } elseif ($test->noOfSkillsPassed > 0){
            $highest_passed = $this->tracksPassed()->max('level_id') ? $this->tracksPassed()->max('level_id') : (int)($this->maxile_level/100) ;
        ...
        if ($this->maxile_level>600){
            ...
            Mail::send([],[], function ($message) use ($note) {
                $message->from(env("MAIL_ORDER_ADDRESS"), 'All Gifted Admin')
                        ->to('ace.allgifted@gmail.com')->cc('info.allgifted@gmail.com')
